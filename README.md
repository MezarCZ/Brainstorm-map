...
<head>
    <meta charset="UTF-8">
    <title>Brainstorming Neon Pro - Shortcuts</title>
    <style>
        body { 
        ...
        .hint { position: fixed; bottom: 20px; left: 20px; color: #888; font-size: 0.85em; background: rgba(0,0,0,0.7); padding: 10px 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); }
    </style>
</head>
<body>
    ...
    <script>
        ...
        document.getElementById('hint-box').textContent = "Enter: Dal\u0161\u00ED my\u0161lenka | Delete: Smazat | Esc: Zru\u0161it v\u00FDb\u011Br";
        ...
        function createBubbleElement(x, y, text, color) {
            ...
            node.style.top = y + 'px';
            node.style.background = color + "CC";
            ...
            // Logika pro klávesnici v bublině
            node.onkeydown = (e) => {
        ...
        function addNode() {
            ...
            const y = (window.innerHeight / 2 - posY) / scale + 80; // Posunuto o kus níž
        ...
        window.onkeydown = (e) => {
            if (e.key === "Delete" || e.key === "Backspace") {
                ...
                if (active.classList.contains('bubble')) {
                    deleteNode(active);
                }
            }
        };
        ...
        function exportToImage() {
            ...
            nodes.forEach(node => {
                if (node.parent) {
                    ...
                    eCtx.shadowBlur = 10; eCtx.shadowColor = node.color; eCtx.strokeStyle = node.color;
            ...
            nodes.forEach(node => {
                ...
                eCtx.fillStyle = node.color; eCtx.shadowBlur = 15; eCtx.shadowColor = node.color;
                eCtx.beginPath(); eCtx.roundRect(x, y, w, h, 16); eCtx.fill();
                eCtx.shadowBlur = 0; eCtx.fillStyle = "white"; eCtx.font = "bold 16px sans-serif";
                eCtx.textAlign = "center"; eCtx.textBaseline = "middle";
                eCtx.fillText(node.el.innerText, x + w / 2, y + h / 2);
            });

            const link = document.createElement('a');
            link.download = 'brainstorm-shortcuts.png';
        ...
        function saveToLocalStorage() {
            ...
            localStorage.setItem('myShortcutsMap', JSON.stringify(data));
        }

        function exportToFile() {
            saveToLocalStorage();
            const data = localStorage.getItem('myShortcutsMap');
        ...
        window.onload = () => {
            const saved = localStorage.getItem('myShortcutsMap');
